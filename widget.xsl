...
<xsl:template name="properties">
    ...
    <xsl:when test="@type = 'entity' or @type = 'microflow' or @type = 'entityConstraint' or @type = 'attribute' or @type = 'form' or @type ='image' or @type = 'translatableString'">string;
    </xsl:when>
    <xsl:when test="@type = 'integer'">number;
    </xsl:when>
    <xsl:when test="@type = 'object'">Array&lt;{
    <xsl:for-each select="m:properties/m:property">
    <xsl:call-template name="properties" />
    </xsl:for-each>}&gt;;
    </xsl:when>
    <xsl:when test="@type = 'enumeration'"><xsl:for-each select="m:enumerationValues/m:enumerationValue">"<xsl:value-of select="@key" />"<xsl:if test="position() != last()"> | </xsl:if> </xsl:for-each>;
    </xsl:when>
